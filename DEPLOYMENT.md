...
Templates use **relative links** (e.g. `../static/style.css` or `Action=""`) to ensure the
UI works correctly at any sub-path, even if `root_path` is misconfigured.

To support this, every route handler must pass a `root_rel` context variable to the
template, indicating the relative path back to the application root:
* Root routes (e.g. `/`) use `.`
* Nested routes (e.g. `/secret/{uuid}`) use `..`
* Deeper routes would use `../..`, etc.

However, `root_path` is **still required** so that the "Share this link" URL
generated by the backend includes the correct prefix (e.g. `/burnar/secret/...`).
